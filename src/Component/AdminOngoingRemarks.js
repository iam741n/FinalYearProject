...
fetch(`http://${API_IP}/WaitingQueue/api/QueueHandler/OnTopByProject?meetingid=31&projectname=${projectNameInput}&dated=2024-01-16`)
.then((response) => response.json())
.then((data) => {
...
// const meetingId = 31; // Replace with your actual meeting ID
// const gender = 'F'; // Replace with your actual gender value
...
// fetch(`http://${API_IP}/WaitingQueue/api/QueueHandler/OnTopByGender?meetingid=${meetingId}&gender=F&dated=2024-01-16`)
// .then((response) => response.json())
// .then((data) => {
// console.log('API Response:', data);
...
fetch(`http://${API_IP}/WaitingQueue/api/QueueHandler/OnTopBySupervisor?meetingid=${meetingId}&supervisorId=${supervisorId}&dated=2024-01-16`)
.then((response) => response.json())
.then((data) => {
console.log('API Response:', data);
...
const handleGroupMembersSubmit = () => {
    // Use studentRemarks to get remarks for each student
    const remarksPayload = Object.entries(studentRemarks).map(([studentId, remarks]) => ({
        id: studentId,
        remarks,
    }));
// Prepare the payload for the API call
// Prepare the payload for the API call
//const combinedPayload = [...unselectedGroupMembers, ...remarksPayload];
...
// Send a POST request to the API endpoint
  fetch(`http://${API_IP}/WaitingQueue/api/Supervisor/SaveGroupMembersRemark?mId=${yourMeetingId}&gid=${ga_id}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
<Modal.Body>
  {groupInfo && (
    <div>
      ...
      {/* Display group members with textboxes for remarks */}
      {groupInfo.GroupMembers.map((member) => (
        <div key={member.student_id}>
          <label htmlFor={`remarks-${member.student_id}`}>
            {member.name} - {member.Regno}:
          </label>
          <input
            type="text"
            id={`remarks-${member.student_id}`}
            value={studentRemarks[member.student_id] || ''}
            onChange={(e) => handleRemarksChange(member.student_id, e.target.value)}
          />
        </div>
      ))}
    </div>
...
<div className="d-flex flex-column align-items-center">
      <h5>Select Meeting Date <IoCalendarNumberOutline /></h5>
      <DatePicker
...
export default AdminOngoingRemarks;